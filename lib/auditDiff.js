...
function migrateAuditEntry(entry) {
  ...
  const originalOperator = entry.operator;
  if (!entry.operator) { entry.operator = "unknown"; changed = true; }
  if (!entry.operatorName) { entry.operatorName = originalOperator ? (ROLE_INFO[originalOperator]?.name || originalOperator) : "未知"; changed = true; }
